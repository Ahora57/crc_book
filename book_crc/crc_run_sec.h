...
namespace crc_sec
{
	...
	private:
		...
		std::vector<CRC_INFO> crc_list;
         
        
        //https://github.com/gityf/crc/blob/ac358396a20c77cf664076c5844e656acb5978f2/crc/crc32.c#L72
        auto crc32(CRC_INFO* crc_info) -> uint32_t
        { 
            uint32_t crc_res = ~NULL; 

            for (uint32_t i = NULL; i < crc_info->crc_size; i++)
            {
                crc_res = crc32_tab[(crc_res ^ crc_info->crc_addr[i]) & 0xFF] ^ ((crc_res >> 8) & 0x00FFFFFF);
            }

            return ~crc_res;
        }

        auto crc_unhook_code(CRC_INFO* crc_info) -> VOID
        {
            DWORD old_prot = NULL;
            if (VirtualProtect(crc_info->crc_addr, crc_info->crc_size,PAGE_EXECUTE_READWRITE,&old_prot))
            { 
                if (crc_info->crc_steals_check)
                {
                    WriteProcessMemory(NtCurrentProcess, crc_info->crc_addr, crc_info->crc_coped_mem, crc_info->crc_size, NULL);
                }
                else
                {
                    memcpy(crc_info->crc_addr, crc_info->crc_coped_mem, crc_info->crc_size);
                }
                VirtualProtect(crc_info->crc_addr, crc_info->crc_size, old_prot, &old_prot);
            }
        }
